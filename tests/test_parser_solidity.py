...
contract Vault is Ownable {
    ...
    function viewTotal() external view returns (uint256) { return total; }
}
"""
...
def _skip_if_error(res):
    """Если solc/скачивание недоступны в окружении CI — пропускаем тесты,
    иначе падаем на реальных ошибках парсинга."""
    if res["status"] == "ok":
        return
    # если ошибка из-за отсутствия сети/solc — пропустим
    msg = " ".join(res.get("errors", []))
    known_env_issues = ("ConnectionError", "SSLError", "Timeout", "MaxRetryError", "SolcNotInstalled")
    if any(k in msg for k in known_env_issues):
        pytest.skip(f"Solc/Network not available in test env: {msg}")
    pytest.fail(f"Unexpected parser error: {msg}")


def test_empty_input_error():
    res = run("", engine="solc", auto_version=True)
    assert res["status"] == "error"
    assert "EmptyInput" in res.get("errors", [])
...
def test_extract_contracts_and_functions():
    ...
    withdraw = next(f for f in fns if f["name"] == "withdraw")
    assert "onlyOwner" in withdraw.get("modifiers", [])
    assert withdraw["visibility"] in {"public", "external"}
    assert withdraw["stateMutability"] in {"nonpayable", "payable"}  

    view_total = next(f for f in fns if f["name"] == "viewTotal")
    assert "returns" in view_total["signature"]
    assert "uint256" in view_total["signature"]
